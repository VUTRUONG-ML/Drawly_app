...
import { View, StyleSheet, GestureResponderEvent, TouchableOpacity, ToastAndroid } from 'react-native';
...
const DrawingCanvas = forwardRef(({
    ...
    if (userId && drawId) {
      try {
        ...
        const imageUrls = updated
          ...
          .map(img => (img as ImageShape).uri);
        
        await updateDraw(drawId, updated, imageUrls);
  ...
  useImperativeHandle(ref, () => ({
    ...
    addImage: async (uri: string, x: number = 100, y: number = 100) => {
      try {
